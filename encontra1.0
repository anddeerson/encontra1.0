...
def main():
    st.title("Encontra aluno(s) aprovado(s) versão 1.0")
    st.write("Cole a lista de nome(s) do(s) aluno(s) no campo abaixo e carregue um ou mais PDFs dos aprovados.")
    ...
    if nomes_texto and pdf_files:
        ...
        if results:
            st.success("Aluno(s) aprovado(s) encontrado(s)!")
            ...
            st.write(f"Total de aluno(s) na lista: {total_nomes}")
            st.write(f"Total de aprovado(s) encontrados: {total_aprovados}")
